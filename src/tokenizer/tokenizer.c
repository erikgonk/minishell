...
/*   Updated: 2024/08/12 11:46:09 by vaunevik         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	token_length(char *input)
{
	...
	length = 0;
	add = 1;
	if ((input[0] == C_LESS && input[1] == C_LESS) || (input[0] == C_GREAT
			&& input[1] == C_GREAT))
		return (2);
	else if (input[0] == C_PIPE || input[0] == C_GREAT || input[0] == C_LESS)
		return (1);
	while (input[length] && input[length] != C_PIPE && input[length] != C_LESS
		&& input[length] != C_GREAT && input[length] != ' ')
	{
